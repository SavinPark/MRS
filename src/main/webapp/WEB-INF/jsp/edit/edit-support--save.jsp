...
<body>
	...
	<c:if test="${employee != null}">
		...
		<!-- 내용 -->
			...
			<!-- 보고서 등록 Form & Save 버튼 -->
			<!-- /MRStest/support/updateAction -->
			<form class="container" method="POST" action="">
				<input type="text" name="support_idx" id="support_idx" value="" />
				<input type="text" name="customer_id" id="customer_id" value="" />
				<input type="text" name="employee_id" id="employee_id" value="" />
				<input type="text" name="support_type" id="support_type" value="" />
				<input type="text" name="support_purpose" id="support_purpose" value="" />
				<input type="text" name="support_date" id="support_date" value="" />
				<input type="text" name="support_number_of_files" id="support_number_of_files" value="" />
				<input type="text" name="board_idx" id="board_idx" value="" />
				<input type="text" name="board_title" id="board_title" value="" />
				<input type="text" name="board_comment" id="board_comment" value="" />
	...
	<script type="text/javascript">
		...
		/* :: 초깃값 세팅 :: */
		$("#custCompanyCombo").val("${custInfoVO.customer_company}").prop("selected",true); // 고객사 선택
		getDepartmentData(); // 부서 옵션 가져오기
		$("#custDepartmentCombo").val("${custInfoVO.customer_department}").prop("selected",true); // 부서 
		getManagerData(); // 담당자 옵션 가져오기
	//	$("#custManagerCombo").val("${custInfoVO.customer_manager}").prop("selected",true); // 담당자
		// getCustomerId();
		$("#sprtTypeCombo").val("${sprtInfoVO.support_type}").prop("selected",true); // 지원유형
		$("#sprtPurposeCombo").val("${sprtInfoVO.support_purpose}").prop("selected",true); // 지원목적
		$("#empCombo").val("${empInfoVO.employee_id}").prop("selected",true); // 지원SE
