...
<body>
	...
	<script type="text/javascript">
		...
		// URL Params
		let board_idx = urlParmas.get("board_idx"); // board_idx
		let customer_company = urlParams.get("customer_company");
		let customer_department = urlParams.get("customer_department");
		let customer_manager = urlParams.get("cuatomer_manager");
		*/
		...
		function getDepartments() {
			$.ajax({ // 컨트롤러와 통신
				type: 'POST',
				url: "support/add/getDepartments",
				data: { company : $("#custCompanyCombo option:selected").val() },
				...
				complete: function() {
					alert($("#custCompanyCombo option:selected").val());
				}
			});
		}
		// 부서 선택 후
		function getManagers() {
			if($("#custCompanyCombo").val() == '') {
				alert('고객사를 선택하세요.');
				$("#custCompanyCombo").focus();
				return;
			}
			var jqxhr = $.post("add/getManagers", 
						{customer_company : $("#customer_company").val(),
						 customer_department : $("#customer_department").val()})
			.done(function() { alert("2nd Success"); })
